...
import net.mcreator.explosivegg.world.biome.SkeletonBiomeBiome;
...
public class ExplosiveggModBiomes {
	public static final DeferredRegister<Biome> REGISTRY = DeferredRegister.create(ForgeRegistries.BIOMES, ExplosiveggMod.MODID);
	public static final RegistryObject<Biome> SKELETON_BIOME = REGISTRY.register("skeleton_biome", () -> SkeletonBiomeBiome.createBiome());

	@SubscribeEvent
	public static void init(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			SkeletonBiomeBiome.init();
		});
	}
			...
			for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : worldGenSettings.dimensions().entrySet()) {
				...
				if (dimensionType == dimensionTypeRegistry.getOrThrow(DimensionType.OVERWORLD_LOCATION)) {
					...
					if (chunkGenerator.getBiomeSource() instanceof MultiNoiseBiomeSource noiseSource) {
						...
						parameters.add(new Pair<>(SkeletonBiomeBiome.PARAMETER_POINT,
								biomeRegistry.getOrCreateHolder(ResourceKey.create(Registry.BIOME_REGISTRY, SKELETON_BIOME.getId()))));
					...
					if (chunkGenerator instanceof NoiseBasedChunkGenerator noiseGenerator) {
						...
						if (currentRuleSource instanceof SurfaceRules.SequenceRuleSource sequenceRuleSource) {
							...
							surfaceRules.add(1, preliminarySurfaceRule(ResourceKey.create(Registry.BIOME_REGISTRY, SKELETON_BIOME.getId()),
									Blocks.GRASS_BLOCK.defaultBlockState(), Blocks.DIORITE.defaultBlockState(), Blocks.DIORITE.defaultBlockState()));
