...
package net.mcreator.explosivegg.keybind;
...
import net.mcreator.explosivegg.procedures.GOnKeyPressedProcedure;
...
public class InvisKeyBinding extends ExplosiveggModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;

	public InvisKeyBinding(ExplosiveggModElements instance) {
		super(instance, 22);
	...
	public void initElements() {
		keys = new KeyBinding("key.explosivegg.invis", GLFW.GLFW_KEY_DOWN, "key.categories.misc");
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					ExplosiveggMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
	...
	private static void pressAction(PlayerEntity entity, int type, int pressedms) {
		...
		if (type == 0) {
			...
			GOnKeyPressedProcedure.executeProcedure(Stream
